...
#import "XMLocationTrans.h"
#import <UIKit/UIGeometry.h>
#include <math.h>

@implementation XMLocationTrans
...
+ (CLLocationCoordinate2D)getBaiduFromGPS:(CLLocationCoordinate2D)coordinate {
    CLLocationCoordinate2D marsCoordinate = [self getMarsFromGPS:coordinate];
    return [self getBaiduFromMars:marsCoordinate];
}

/// 获取【GPS】坐标（WGS-84） 传入：百度坐标（BD-09）
+ (CLLocationCoordinate2D)getGPSFromBaidu:(CLLocationCoordinate2D)coordinate {
    CLLocationCoordinate2D marsCoordinate = [self getMarsFromBaidu:coordinate];
    return [self getGPSFromMars:marsCoordinate];
}
